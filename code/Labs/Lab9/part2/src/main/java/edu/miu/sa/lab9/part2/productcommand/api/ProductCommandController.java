package edu.miu.sa.lab9.part2.productcommand.api;
...
public class ProductCommandController {
    ...
    @PostMapping
    public ProductSnapshot createProduct(@Valid @RequestBody ProductRequest request) {
        return productCommandService.addProduct(request.productNumber(), request.name(), request.price());
    }

    @PutMapping("/{productNumber}")
    public ProductSnapshot updateProduct(@PathVariable String productNumber, @Valid @RequestBody ProductUpdateRequest request) {
